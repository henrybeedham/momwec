...
export class Board {
  ...
  private initializeSquares(): Square[] {
    ...
    // Stations
    squares.push(
      new StationSquare(5, "King's Cross Station", 200, [25, 50, 100, 200]),
    );
    squares.push(
      new StationSquare(15, "Marylebone Station", 200, [25, 50, 100, 200]),
    );
    squares.push(
      new StationSquare(25, "Fenchurch St. Station", 200, [25, 50, 100, 200]),
    );
    squares.push(
      new StationSquare(
        35,
    ...
    try {
      ...
      this.squares = json.squares.map((square) => {
        switch (square.type) {
          ...
          case "station":
            return new StationSquare(
              (square as StationSquare).id,
              (square as StationSquare).name,
              (square as StationSquare).price,
              (square as StationSquare).rent,
            );
          case "utility":
            return new UtilitySquare(
              (square as UtilitySquare).id,
              (square as UtilitySquare).name,
              (square as UtilitySquare).price,
              (square as UtilitySquare).multipliers,
            );
          case "corner":
            return new CornerSquare(square.id, square.name);
          case "tax":
            return new TaxSquare(
              square.id,
              square.name,
